...
1. **Tier 1 (Fast-Path)**: For bug fixes, localized refactors, minor features, internal helpers, documentation, or config edits, run targeted commands on the affected package:
   ...
   - Short test execution: `go test -short ./path/to/affected/package`
...
2. **Tier 2 (Full Verification)**: For core architectural modifications, concurrency invariants, durable storage schemas, public package APIs, or release gates:
   ...
   - Module hygiene check: `git diff -- go.mod go.sum  # inspect dependency changes; tidy only when relevant`
...
- Package layout, `internal/` boundaries, consumer interface design, Concrete-First Design, constructor idioms: read [references/architecture-packages.md](references/architecture-packages.md).
