...
def collect_timing_samples(cmd: str, runs: int, warmups: int) -> List[float]:
    ...
    for _ in range(runs):
        ...
        if res.returncode != 0:
            raise RuntimeError(f"Command '{cmd}' failed with return code {res.returncode}: {res.stderr.decode('utf-8', errors='ignore')}")
        samples.append((end - start) * 1000.0) # Convert to ms
    return samples


def bootstrap_speedup_ci(base_samples: List[float], opt_samples: List[float], iterations: int = 2000) -> Tuple[float, float]:
    ...
    for _ in range(iterations):
        b_resample = [random.choice(base_samples) for _ in range(n_b)]
        o_resample = [random.choice(opt_samples) for _ in range(n_o)]
        b_med = sorted(b_resample)[n_b // 2]
        o_med = sorted(o_resample)[n_o // 2]
        if o_med > 0:
...
def compare_benchmarks(
    ...
    print(f"[*] Running Baseline Benchmark ({runs} runs, {warmups} warmups)...")
    base_samples = collect_timing_samples(baseline_cmd, runs, warmups)
    base_stats = compute_statistics(base_samples)

    print(f"[*] Running Optimized Benchmark ({runs} runs, {warmups} warmups)...")
    opt_samples = collect_timing_samples(optimized_cmd, runs, warmups)
    opt_stats = compute_statistics(opt_samples)

    speedup = base_stats["median_ms"] / opt_stats["median_ms"] if opt_stats["median_ms"] > 0 else 0.0
    ci_low, ci_high = bootstrap_speedup_ci(base_samples, opt_samples)
    latency_reduction_pct = ((base_stats["median_ms"] - opt_stats["median_ms"]) / base_stats["median_ms"]) * 100.0 if base_stats["median_ms"] > 0 else 0.0
    ...
    print("=" * 70)
    print(f"{'Metric':<25} | {'Baseline':<18} | {'Optimized':<18} | {'Improvement':<12}")
    print("-" * 70)
    print(f"{'Median Latency':<25} | {base_stats['median_ms']:>14.3f} ms | {opt_stats['median_ms']:>14.3f} ms | {speedup:>10.2f}x")
    print(f"{'Mean Latency':<25} | {base_stats['mean_ms']:>14.3f} ms | {opt_stats['mean_ms']:>14.3f} ms | {base_stats['mean_ms']/opt_stats['mean_ms']:>10.2f}x")
    print(f"{'Std Deviation':<25} | {base_stats['std_dev_ms']:>14.3f} ms | {opt_stats['std_dev_ms']:>14.3f} ms | {'-':>12}")
    print(f"{'p95 Latency':<25} | {base_stats['p95_ms']:>14.3f} ms | {opt_stats['p95_ms']:>14.3f} ms | {base_stats['p95_ms']/opt_stats['p95_ms']:>10.2f}x")
    print(f"{'Min Latency':<25} | {base_stats['min_ms']:>14.3f} ms | {opt_stats['min_ms']:>14.3f} ms | {base_stats['min_ms']/opt_stats['min_ms']:>10.2f}x")
    print("-" * 70)
...
def run_internal_self_test():
    ...
    assert abs(stats["median_ms"] - 10.95) < 0.1, f"Median incorrect: {stats['median_ms']}"
...
def main():
    ...
    parser.add_argument("--runs", type=int, default=15, help="Number of benchmark sample runs")
    parser.add_argument("--warmups", type=int, default=3, help="Number of warmup executions")
    parser.add_argument("--json-output", help="Path to write benchmark results JSON")
    parser.add_argument("--test-internal", action="store_true", help="Run internal statistical self-test")
