...
class WorktreeTests(Repository):
    ...
    def test_linked_worktree_uses_own_baseline_cache_lock_and_tools(self):
        ...
        with lock_path.open("a") as lock:
            ...
            self.assertEqual(busy["decision"], "block")
            self.assertIn("already running", busy["reason"])
            blocked = self.hook("Stop", stop_hook_active=True)
            self.assertNotIn("decision", blocked)
            self.assertIn("report the blocker", blocked["systemMessage"])
        ...
        self.assertIn("not a pass", missing["reason"])
        self.assertEqual((self.root / "calls").read_text(), "checked\n")
